...
def test_build_report2_excel_outputs_workbook() -> None:
    ...
    assert sheet.page_margins.top == 1
    assert sheet.page_margins.bottom == 1
    assert sheet.page_margins.left == 0.3
    assert sheet.page_margins.right == 0.3
    assert sheet.page_margins.header == 1.9
    assert sheet.page_margins.footer == 0.6
