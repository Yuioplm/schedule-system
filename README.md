...
このリポジトリは、**外来スケジュールの枠管理・予定変更・反映後予定検索・変更履歴編集・帳票出力・マスタ管理・ドキュメント閲覧**を行う、
...
- 帳票②表示制御は `M_ClinicalDepartment.Rpt2Flag` に加え、予定変更入力（通常枠変更）の `hide_from_report2` と臨時外来登録の `temp_hide_from_report2` によって保存される `T_ScheduleChange.Rpt2Flag` / `T_TemporarySchedule.Rpt2Flag` の影響を受ける
