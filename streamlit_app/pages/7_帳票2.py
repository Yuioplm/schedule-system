...
st.caption(
    "前回の正式出力履歴と今回の帳票②出力内容を比較し、"
    "新規・更新行を水色かつ太字で出力します。お試し出力は次回差分判定の基準にしません。"
)
...
if diff_df.empty:
    ...
    trial_file_name = f"帳票②_予定変更一覧_確認用_{date.today():%Y%m%d}.xlsx"
    official_file_name = f"帳票②_予定変更一覧_正式_{date.today():%Y%m%d}.xlsx"
    trial_excel = build_report2_excel(diff_df, "確認用")
    official_excel = build_report2_excel(diff_df, "正式")

    col1, col2 = st.columns(2)
    with col1:
        st.download_button(
            label="お試しExcelダウンロード（履歴に残さない）",
            data=trial_excel,
            file_name=trial_file_name,
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
    with col2:
        output_by = st.text_input("正式出力者（任意）", value="")
        official_clicked = st.download_button(
            label="正式Excelダウンロード（履歴に登録）",
            data=official_excel,
        ...
        if official_clicked:
            ...
            st.success(f"正式出力履歴を登録しました（履歴ID: {history_id}）。")

with st.expander("正式出力履歴"):
    ...
    if history_df.empty:
        ...
        if st.button("選択した正式出力履歴を取消"):
            if cancel_target is None:
                st.warning("取消する正式出力履歴IDを選択してください。")
                ...
                conn.commit()
                st.success("正式出力履歴を取消しました。再読み込み後、差分基準から除外されます。")
