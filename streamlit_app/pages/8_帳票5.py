...
with col1:
    fiscal_year = st.selectbox("年度", years, index=1)
with col2:
    month = st.selectbox("月", months, index=datetime.now().month - 1)

start_date = f"{fiscal_year}-04-01"
end_year = fiscal_year + 1 if month < 4 else fiscal_year
date = f"{end_year}-{month:02d}-01"
...
if not df.empty:
    ...
    st.download_button(
        label="Excelダウンロード",
        data=csv,
        file_name=f"帳票➄_{fiscal_year}.csv",
        mime="text/csv",
    )
else:
